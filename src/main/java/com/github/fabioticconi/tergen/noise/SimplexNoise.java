...
{ // Simplex noise in 2D, 3D and 4D
    ...
    private static       short  p[]         = { 151, 160, 137, 91, 90, 15, 131, 13, 201, 95, 96, 53, 194, 233, 7, 225,
    ...
    public static double noise(final double xin, final double yin)
    {
        ...
        // c = (3-sqrt(3))/6
        final double x1 = x0 - i1 + G2; // Offsets for middle corner in (x,y)
        // unskewed coords
        final double y1 = y0 - j1 + G2;
        final double x2 = x0 - 1.0 + 2.0 * G2; // Offsets for last corner in
        // (x,y) unskewed coords
    ...
    public static double noise(final double xin, final double yin, final double zin)
    {
        ...
        // Skew the input space to determine which simplex cell we're in
        final double s = (xin + yin + zin) * F3; // Very nice and simple skew
        // factor for 3D
        final int    i  = fastfloor(xin + s);
        final int    j  = fastfloor(yin + s);
        ...
        // where
        // c = 1/6.
        final double x1 = x0 - i1 + G3; // Offsets for second corner in (x,y,z)
        // coords
        final double y1 = y0 - j1 + G3;
        final double z1 = z0 - k1 + G3;
        final double x2 = x0 - i2 + 2.0 * G3; // Offsets for third corner in
        // (x,y,z) coords
        final double y2 = y0 - j2 + 2.0 * G3;
        final double z2 = z0 - k2 + 2.0 * G3;
        final double x3 = x0 - 1.0 + 3.0 * G3; // Offsets for last corner in
        // (x,y,z) coords
    ...
    public static double noise(final double x, final double y, final double z, final double w)
    {
        ...
        final double X0 = i - t; // Unskew the cell origin back to (x,y,z,w)
        // space
        final double Y0 = j - t;
        final double Z0 = k - t;
        final double W0 = l - t;
        ...
        // compute that.
        final double x1 = x0 - i1 + G4; // Offsets for second corner in
        // (x,y,z,w) coords
        final double y1 = y0 - j1 + G4;
        final double z1 = z0 - k1 + G4;
        final double w1 = w0 - l1 + G4;
        final double x2 = x0 - i2 + 2.0 * G4; // Offsets for third corner in
        // (x,y,z,w) coords
        ...
        final double x4 = x0 - 1.0 + 4.0 * G4; // Offsets for last corner in
        // (x,y,z,w) coords
